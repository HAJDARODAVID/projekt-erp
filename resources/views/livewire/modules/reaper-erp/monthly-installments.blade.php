    ...
    <x-ui.card :border=FALSE :noBodyPadding=TRUE class="h-100 d-flex flex-column">
        <div class="row flex-fill g-0 gap-3">
            <div class="col-md-3 d-flex flex-column">
                <x-ui.card title="{{ translator('All installments') }}" class="flex-fill d-flex flex-column" headerHight=48>
                ...
                <x-ui.card  class="flex-fill d-flex flex-column" headerHight=48 loading='selectWorker, selectTab'>
                    <x-slot:title>
                        <div class="d-flex gap-2 align-items-center">
                            <div class="">{{ translator('Basic info') }}</div>
                        </div>
                    ...
                    @if ($selectedInstallment)
                        testi
                    @else
                        <div class="d-flex justify-content-center"><div class="py-3"><i>{{ translator('No installment selected!') }}</i></div></div>
                    @endif
                </x-ui.card>
            </div>
        </div>
